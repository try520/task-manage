...
from apscheduler.events import EVENT_SCHEDULER_PAUSED,EVENT_SCHEDULER_RESUMED,EVENT_JOB_EXECUTED,EVENT_JOB_ERROR,EVENT_JOB_MISSED
...
# 调度器配置
job_defaults = {
    'coalesce': False,#当由于某种原因导致某个job积攒了好几次没有实际运行（比如说系统挂了5分钟后恢复，有一个任务是每分钟跑一次的，按道理说这5分钟内本来是“计划”运行5次的，但实际没有执行），如果coalesce为True，下次这个job被submit给executor时，只会执行1次，也就是最后这次，如果为False，那么会执行5次
    ...
    'misfire_grace_time': 1 #设想和上述coalesce类似的场景，如果一个job本来14:00有一次执行，但是由于某种原因没有被调度上，现在14:01了，这个14:00的运行实例被提交时，会检查它预订运行的时间和当下时间的差值（这里是1分钟），大于我们设置的30秒限制，那么这个运行实例不会被执行。
}
...
class Task(object):
	...
	def __init__(self):
		...
		if os.path.exists(self.taskRootDir)==False:
			os.makedirs(self.taskRootDir)
		self.scheduler = BackgroundScheduler( executors=executors, job_defaults=job_defaults, daemonic=True) #实例化定时任务调度对象(非阻塞方式)
	...
	def getItems(self):
		if len(self.taskItems)==0:
			dirList = os.listdir(self.taskRootDir)  # 列出文件夹下所有的目录与文件
	...
	def runTask(self,taskItem):
		...
		cmd=""
		if taskItem['path']!=None and taskItem['path']!='':
	...
	def removeLog(self,taskItem):
		logDir=os.path.join(self.taskRootDir,taskItem['name'],'log')
	...
	def sigint_handler(self,signum, frame):
		...
		self_pid = os.getpid()
		try:
			pidFilePath=os.path.join(self.taskRootDir,'pid')
